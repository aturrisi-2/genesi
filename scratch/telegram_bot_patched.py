...
async def _try_extract_faces_from_text(text: str, tmp_img: str, desc_img: str, session_uid: str) -> bool:
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).warning("Error parsing faces names: %s", e)
    ...
    try:
        ...
        if photo:
            ...
            if analysis and analysis != "__TOKEN_EXPIRED__":
                ...
                if "[UNKNOWN_FACES_DETECTED]" in analysis:
                    if faces_saved_now:
                        user_msg += "\n[SISTEMA: Hai estratto e memorizzato con successo le identità dei volti dalla didascalia dell'utente. Ringrazia l'utente per avertele presentate!]"
                    else:
                        user_msg += "\n[SISTEMA: Hai rilevato persone sconosciute nell'immagine. Chiedi all'utente in modo colloquiale e naturale chi sono le persone nella foto, così potrai memorizzarle.]"
        ...
        if session.get("awaiting_faces_img"):
            ...
            if faces_saved:
                ...
                text += "\n\n[SISTEMA: Hai estratto e memorizzato con successo le identità di queste persone. Esclama in modo naturale che ti ricorderai di loro!]"
