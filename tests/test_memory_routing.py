"""
Tests for Memory Routing - conversational memory references
Tests the memory routing override and memory_context responses.
"""

import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import asyncio
import pytest
from unittest.mock import AsyncMock, patch, MagicMock

from core.proactor import Proactor, is_memory_reference
from core.chat_memory import chat_memory
from core.memory_storage import memory_storage
from core.storage import storage


# ═══════════════════════════════════════════════════════════════
# Test: Memory Reference Detection
# ═══════════════════════════════════════════════════════════════

class TestMemoryReferenceDetection:

    def test_memory_reference_patterns(self):
        """
        Frasi che indicano esplicitamente la memoria conversazionale.
        I trigger sono deliberatamente specifici (commit 8d9c445): parole nude
        come 'prima', 'ieri', 'ricordi' NON bastano — servono per evitare
        collisioni con identity ('come mi chiamo') e reminder ('mi ricordi di...').
        """
        memory_phrases = [
            "cosa abbiamo detto",
            "cosa dicevamo",
            "di cosa abbiamo parlato",
            "ci siamo detti",
            "avevamo detto",
            "abbiamo parlato",
            "avevamo parlato",
            "l'altra volta",
            "discusso",
            "di cosa parlavamo",
            "dicevi prima",
            "ricordi cosa ci siamo detti",
        ]

        for phrase in memory_phrases:
            assert is_memory_reference(phrase), f"Should detect: {phrase}"
            assert is_memory_reference(f"Tu {phrase}?"), f"Should detect: {phrase}"

        # 'ricordi'/'ti ricordi' richiede un riferimento temporale esplicito
        assert is_memory_reference("ti ricordi prima?")
        assert is_memory_reference("ricordi ieri?")
        assert is_memory_reference("ti ricordi cosa dicevamo?")

    def test_memory_reference_case_insensitive(self):
        """Test case insensitive detection."""
        assert is_memory_reference("RICORDI COSA CI SIAMO DETTI?")
        assert is_memory_reference("Ricordi AVEVAMO PARLATO?")
        assert is_memory_reference("DI COSA ABBIAMO PARLATO?")

    def test_non_memory_phrases(self):
        """Test that non-memory phrases are not detected."""
        non_memory = [
            "che tempo fa",
            "notizie di oggi",
            "come stai",
            "ciao",
            "aiutami",
            "spiegami",
            "cos'è",
            "dove si trova",
            # Anti-collisione: identity e reminder NON sono memory_context
            "come mi chiamo",
            "prima di noi?",
            "ricordi il mio nome?",
        ]

        for phrase in non_memory:
            assert not is_memory_reference(phrase), f"Should NOT detect: {phrase}"

    def test_partial_matches(self):
        """Test partial matches work correctly."""
        assert is_memory_reference("di cosa abbiamo parlato prima?")
        assert is_memory_reference("ieri abbiamo parlato del mare")
        assert is_memory_reference("ricordi cosa ci siamo detti?")
        assert is_memory_reference("ti ricordi cosa dicevamo ieri?")


# ═══════════════════════════════════════════════════════════════
# Test: Memory Routing Integration
# ═══════════════════════════════════════════════════════════════

class TestMemoryRoutingIntegration:

    @pytest.fixture
    def proactor(self):
        return Proactor()

    @pytest.fixture
    def setup_user_history(self):
        """Setup a user with conversation history."""
        user_id = "test-memory-user"
        
        # Clear any existing data
        memory_storage.delete(f"chat:{user_id}")
        
        # Add 3 conversation messages
        messages = [
            ("ciao", "Ciao! Come posso aiutarti oggi?", "greeting"),
            ("mi chiamo Mario e vivo a Roma", "Ciao Mario! Piacere di conoscerti. Roma è una bella città.", "identity"),
            ("che tempo fa a Roma?", "Oggi a Roma c'è bel tempo, con temperature piacevoli.", "weather")
        ]
        
        for user_msg, sys_msg, intent in messages:
            chat_memory.add_message(user_id, user_msg, sys_msg, intent)
        
        return user_id

    @pytest.mark.asyncio
    async def test_memory_routing_override(self, proactor, setup_user_history):
        """
        Test that memory references trigger memory_context intent.
        NOTA: _handle_memory_context usa _call_model (NON _call_with_protection)
        per preservare il proprio prompt dal prompt adattivo.
        """
        user_id = setup_user_history

        with patch('core.llm_service.llm_service._call_model', new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "Ricordo che ti chiami Mario e vivi a Roma. Abbiamo parlato anche del tempo oggi."

            response = await proactor.handle(user_id, "di cosa abbiamo parlato prima?")

            # Almeno una chiamata deve essere andata sulla route memory
            assert mock_llm.called
            memory_calls = [c for c in mock_llm.call_args_list
                            if c.kwargs.get('route') == "memory"]
            assert memory_calls, "nessuna chiamata _call_model con route='memory'"
            assert memory_calls[0].kwargs.get('user_id') == user_id

            # Response should be memory-aware
            assert "Mario" in response or "Roma" in response or "tempo" in response

    @pytest.mark.asyncio
    async def test_memory_routing_with_no_history(self, proactor):
        """
        Test memory routing with no conversation history.
        Usa un user_id univoco: i run precedenti possono lasciare file di
        chat persistenti per id riusati, falsando chat_count.
        """
        import uuid
        user_id = f"no-history-{uuid.uuid4().hex[:12]}"

        with patch('core.llm_service.llm_service._call_with_protection', new_callable=AsyncMock) as mock_prot:
            mock_prot.return_value = "Di cosa ti piacerebbe parlare?"

            # With no history, the memory OVERRIDE must not fire (chat_count=0).
            # Il classificatore LLM può comunque scegliere memory_context: in tal
            # caso la risposta corretta è l'ammissione onesta di non avere storia.
            response = await proactor.handle(user_id, "di cosa abbiamo parlato?")

        # Should get a valid response, never a fabricated memory
        assert response is not None and len(response) > 0
        # Non deve INVENTARE contenuti di conversazioni mai avvenute
        assert "Mario" not in response and "Laura" not in response

    @pytest.mark.asyncio
    async def test_memory_routing_bypasses_classifier(self, proactor, setup_user_history):
        """Test that memory routing bypasses intent classifier."""
        user_id = setup_user_history

        with patch('core.intent_classifier.intent_classifier.classify') as mock_classify:
            with patch('core.llm_service.llm_service._call_model', new_callable=AsyncMock) as mock_llm:
                mock_llm.return_value = "Ricordo la nostra conversazione..."

                # Memory reference should bypass classifier
                response = await proactor.handle(user_id, "ricordi cosa ci siamo detti?")

                # Classifier should NOT be called
                assert not mock_classify.called

                # LLM should be called with memory route (via _call_model)
                memory_calls = [c for c in mock_llm.call_args_list
                                if c.kwargs.get('route') == "memory"]
                assert memory_calls, "nessuna chiamata _call_model con route='memory'"

    @pytest.mark.asyncio
    async def test_non_memory_uses_normal_routing(self, proactor, setup_user_history):
        """Test that non-memory messages use normal routing."""
        user_id = setup_user_history
        
        with patch('core.intent_classifier.intent_classifier.classify') as mock_classify:
            mock_classify.return_value = "greeting"
            
            # Non-memory message should use normal routing
            response = await proactor.handle(user_id, "come stai?")
            
            # Classifier should be called
            assert mock_classify.called

    @pytest.mark.asyncio
    async def test_memory_context_with_llm_failure(self, proactor, setup_user_history):
        """
        Test memory context response when LLM fails.
        Testa direttamente _handle_memory_context: patchare _call_model
        globalmente romperebbe anche il classificatore.
        """
        user_id = setup_user_history

        with patch('core.llm_service.llm_service._call_model', new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = None  # LLM failure

            response, source = await proactor._handle_memory_context(
                user_id, "di cosa abbiamo parlato?", {"profile": {}}, None
            )

            # Should get fallback response
            assert "ricordo" in response.lower() or "scambi" in response.lower()
            assert source == "memory_context"

    @pytest.mark.asyncio
    async def test_memory_context_error_handling(self, proactor, setup_user_history):
        """Test error handling in memory context."""
        user_id = setup_user_history

        with patch('core.chat_memory.chat_memory.get_messages') as mock_get, \
             patch('core.chat_memory.chat_memory.get_message_count', return_value=3), \
             patch('core.memory_brain.memory_brain.update_brain', new_callable=AsyncMock) as mock_brain, \
             patch.object(proactor.context_assembler, 'build', new_callable=AsyncMock) as mock_ctx_build:
            mock_get.side_effect = Exception("Memory error")
            mock_brain.return_value = {"profile": {}, "latent": {}, "relational": {}}
            mock_ctx_build.return_value = {"summary": "ctx", "profile": {}, "current_message": "di cosa abbiamo detto?"}

            response = await proactor.handle(user_id, "di cosa abbiamo detto?")

            # Should get error fallback from _handle_memory_context's inner try/except
            assert "dispiace" in response.lower() or "problema" in response.lower()

    @pytest.mark.asyncio
    async def test_complete_memory_flow(self, proactor):
        """Test complete flow: new user -> conversation -> memory reference."""
        user_id = "complete-flow-user"
        
        # Clear any existing data
        memory_storage.delete(f"chat:{user_id}")
        
        # Mock di entrambi i path LLM: _call_with_protection per le route
        # conversazionali, _call_model per memory_context (regola d'oro)
        with patch('core.llm_service.llm_service._call_with_protection', new_callable=AsyncMock) as mock_prot, \
             patch('core.llm_service.llm_service._call_model', new_callable=AsyncMock) as mock_model:
            mock_prot.return_value = "Ciao! Piacere di conoscerti."
            mock_model.return_value = "Ciao! Piacere di conoscerti."

            # 1. First message - greeting
            response1 = await proactor.handle(user_id, "ciao")
            assert response1

            # Manually add to chat memory since proactor doesn't automatically save responses
            chat_memory.add_message(user_id, "ciao", response1, "greeting")

            # 2. Second message - identity
            mock_prot.return_value = "Ho capito, grazie per avermelo detto."
            response2 = await proactor.handle(user_id, "mi chiamo Laura")
            assert response2
            chat_memory.add_message(user_id, "mi chiamo Laura", response2, "identity")

            # 3. Third message - weather
            mock_prot.return_value = "Oggi c'è bel tempo."
            response3 = await proactor.handle(user_id, "che tempo fa?")
            assert response3
            chat_memory.add_message(user_id, "che tempo fa?", response3, "weather")

            # 4. Memory reference - should trigger memory routing (via _call_model)
            mock_model.return_value = "Ricordo che ti chiami Laura e abbiamo parlato del tempo."
            response4 = await proactor.handle(user_id, "di cosa abbiamo parlato prima?")

            # Should be memory-aware
            assert "Laura" in response4 or "tempo" in response4
            memory_calls = [c for c in mock_model.call_args_list
                            if c.kwargs.get('route') == "memory"]
            assert memory_calls, "nessuna chiamata _call_model con route='memory'"


# ═══════════════════════════════════════════════════════════════
# Test: Regression - Other Routes Still Work
# ═══════════════════════════════════════════════════════════════

class TestRegressionOtherRoutes:

    @pytest.fixture
    def proactor(self):
        return Proactor()

    @pytest.mark.asyncio
    async def test_greeting_still_works(self, proactor):
        """Test that greeting messages still work normally."""
        user_id = "greeting-user"
        
        with patch('core.llm_service.llm_service._call_with_protection') as mock_llm:
            mock_llm.return_value = "Ciao! Come posso aiutarti?"
            
            response = await proactor.handle(user_id, "ciao")
            
            # Should use normal routing (not memory)
            assert mock_llm.called
            assert mock_llm.call_args[1]['route'] != "memory"

    @pytest.mark.asyncio
    async def test_weather_still_works(self, proactor):
        """Test that weather requests still work."""
        user_id = "weather-user"
        
        with patch('core.tool_services.tool_service.get_weather') as mock_weather:
            mock_weather.return_value = "Oggi c'è sole."
            
            response = await proactor.handle(user_id, "che tempo fa a Roma?")
            
            assert "sole" in response
            assert mock_weather.called

    @pytest.mark.asyncio
    async def test_news_still_works(self, proactor):
        """
        Test that news requests still work.
        needs_live_data intercetta le query news verso live_search:
        va disattivato per testare il tool get_news classico.
        """
        user_id = "news-user"

        with patch('core.live_search_service.needs_live_data', return_value=False), \
             patch('core.tool_services.tool_service.get_news') as mock_news:
            mock_news.return_value = "Ultime notizie:..."

            response = await proactor.handle(user_id, "notizie di oggi")

            # Il tool deve essere stato invocato; la risposta finale passa
            # dalla synthesis LLM quindi non si asserisce il testo esatto
            assert mock_news.called
            assert response is not None and len(response) > 0

    @pytest.mark.asyncio
    async def test_profile_still_works(self, proactor):
        """Test that identity questions still work."""
        user_id = "profile-user"
        
        # Setup a profile for the user
        profile_data = {
            "name": "Mario",
            "city": "Roma",
            "age": 30
        }
        await storage.save(f"profile:{user_id}", profile_data)
        
        # Test actual identity question handling
        response = await proactor.handle(user_id, "come mi chiamo?")
        
        # Should get a response containing the name from the profile
        assert "Mario" in response


if __name__ == "__main__":
    # Run tests
    asyncio.run(pytest.main([__file__, "-v"]))
