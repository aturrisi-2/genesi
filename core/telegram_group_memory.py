...
async def build_group_context(chat_id: int, from_id: int, first_name: str,
    ...
    # Rileva il tono emotivo della conversazione (lutto, umorismo, normale)
    # e iniettalo come blocco prioritario PRIMA di tutto il resto.
    try:
    ...
    if _tone_info.get("prompt_block"):
        lines.append(_tone_info["prompt_block"])
        lines.append("")
    ...
    # Istruzione sui membri anonimi: SOLO se ce ne sono davvero. Altrimenti l'LLM
    # la parafrasava nell'output ("...non conosciamo ancora il nome, altri numeri o
    # account secondari") = context leak BUG#3.
    _has_numeric_member = any(
        m.strip().startswith("+") or m.strip().replace(" ", "").replace("(tel:", "").replace(")", "").replace(".", "").isdigit()
        for m in active_members
    ) if active_members else False
    if _has_numeric_member:
        lines.append("[⚠️ IMPORTANTE: I membri che iniziano con '+' o contengono solo cifre sono partecipanti di cui non conosciamo ancora il nome. NON associarli o confonderli MAI con Rita, Ennio o altri familiari dell'albero genealogico! Non allucinare i loro nomi.]")
    lines.append("")
    ...
    if raw_msgs:
        ...
        # (Il rilevamento della domanda inevasa e l'istruzione prioritaria sono gestiti
        #  negli endpoint di gruppo, così la risposta si rivolge davvero a quella persona.)
