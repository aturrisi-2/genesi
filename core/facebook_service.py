...
class FacebookService:
    ...
    async def auto_login(self) -> bool:
        ...
        try:
            ...
            # Prova prima con i cookie esistenti (dispositivo fidato, salta 2FA).
            # Se il form non appare, pulisce i cookie e riprova (cookie corrotti).
            found_form = False
            for attempt, (clear, login_url) in enumerate([
                (False, "https://www.facebook.com/login"),
                (True,  "https://www.facebook.com/login"),
            ]):
                if clear:
                    await self._context.clear_cookies()
                    _slog("FACEBOOK_AUTO_LOGIN_COOKIES_CLEARED")
                ...
                try:
                    ...
                    _slog("FACEBOOK_AUTO_LOGIN_TRY_NEXT", attempt=attempt, url=self._page.url[:80])
            if not found_form:
    ...
    async def post_to_group(self, group_name: str, content: str) -> dict:
        ...
        try:
            ...
            # Aspetta che appaia il composer (placeholder "Scrivi qualcosa" o "Crea un post")
            try:
                await self._page.wait_for_selector(
                    '[aria-label*="Scrivi qualcosa"], [aria-placeholder*="Scrivi qualcosa"], '
                    '[aria-placeholder*="Crea un post"], [aria-label*="Crea un post"]',
                    timeout=10000
                )
            except Exception:
                pass

            # Dismetti popup di benvenuto/overlay aggiuntivi
            for dismiss_label in ("Non ora", "Chiudi", "Ok", "Continua", "Ignora", "Salta"):
                d = await self._page.query_selector(f'[aria-label="{dismiss_label}"]')
                if d:
                    await self._page.evaluate('el => el.click()', d)
                    await asyncio.sleep(1)
            ...
            if not open_btn:
                open_btn = await self._page.query_selector('[aria-placeholder*="Crea un post"]')
            if not open_btn:
            ...
            try:
                await self._page.wait_for_selector(
                    '[aria-label="Pubblica"], [aria-label="Posta"]', timeout=8000
                )
