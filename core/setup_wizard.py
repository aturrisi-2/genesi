...
async def start_wizard(user_id: str, platform: str) -> str:
    ...
    prompt = (
        ...
        "FONDAMENTALE: Se hai bisogno di informazioni dall'utente (es. password, email, nome app, codice 2FA), "
        "fermati e fai una domanda chiara, terminando la tua risposta ESATTAMENTE con la stringa '[DOMANDA]'.\n"
        "Quando hai finito e hai le credenziali necessarie, "
        "scrivile o aggiornale DIRETTAMENTE nel file '.env' del server usando un tuo tool bash. "
        "Dopo aver scritto nel file .env o aver concluso l'operazione con successo, termina la tua risposta con '[COMPLETATO]'."
    )
