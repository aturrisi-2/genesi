...
def detect_head_entity(term: str) -> str | None:
    ...
    if code_tokens:
        ...
        return (alpha_codes or code_tokens)[0]
    ...
    if len(tokens) >= 2 and tokens[0] not in GENERIC_SINGLE_TERMS:
        return tokens[0]
    return None
...
def infer_canonical_label(terms: list[str]) -> str:
    ...
    ordered_modifiers = [
        ...
        if token != head and token not in _tokens(action or "") and token in set(modifiers)
    ][:3]
    parts = []
    if head:
