...
def _face_match_margin() -> float:
    """Margine anti-ambiguità (ratio test). Se miglior match e secondo miglior
    match (identità diversa) distano MENO di questo margine, il volto è ambiguo
    → lasciato 'sconosciuto'. Default 0.0 = disattivato.
    Alza via env FACE_MATCH_MARGIN (es. 0.08) per ridurre falsi positivi."""
    try:
...
async def analyze_faces_biometric(image_path: str, threshold: float = 0.6) -> dict:
    ...
    try:
        ...
        # Soglia + margine env-tunabili senza redeploy
        _threshold = _face_match_threshold(threshold)
        _margin = _face_match_margin()
        _confident = _face_confident_dist()

        # Greedy 1-to-1 con ratio-test anti-ambiguità
        distances = []
        per_face: dict = {i: [] for i in range(total_faces)}
        ...
        for min_dist, i, name in distances:
            if i not in matched_face_indices and name not in matched_identities:
                if min_dist < _threshold:
                    # Ratio-test: ambiguità → "sconosciuto" è meglio di nome sbagliato
                    _pf = per_face.get(i, [])
                    ...
                    face_to_name[i] = name
                    face_to_confidence[i] = round(max(0.0, 1.0 - min_dist / _threshold), 3)
