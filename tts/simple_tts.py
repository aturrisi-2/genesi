...
1 intent → 1 funzione
Text-to-Speech con XTTS v2 - Alexandra Hisakawa 24000Hz originale vanilla
"""
...
class SimpleTTS:
    ...
    Con filtro emoji per TTS (non per chat) e XTTS v2 vanilla
    """
    ...
    def _init_xtts(self):
        """Inizializza XTTS v2 - CONFIGURAZIONE ORIGINALE"""
        try:
    ...
    def synthesize(self, text: str, output_file: Optional[str] = None) -> str:
        ...
        try:
            ...
            # Sintesi ORIGINALE VANILLA
            self._synthesize_vanilla(output_path, filtered_text)
    ...
    def _synthesize_vanilla(self, output_path: Path, text: str):
        """
        Sintesi ORIGINALE VANILLA - Alexandra Hisakawa 24000Hz
        """
        try:
            ...
            logger.info(f"[VOICE_VANILLA] speaker={VOICE_SPEAKER} lang={VOICE_LANGUAGE} sr={VOICE_SAMPLE_RATE}Hz duration={duration:.2f}s samples={len(wav)}")
            
        except Exception as e:
            logger.error(f"Vanilla synthesis failed: {e}")
            raise
